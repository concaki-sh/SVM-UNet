...
class BceDiceLoss(nn.Module):
    def __init__(self, wb=1, wd=1, num_classes=2):
    ...
    def forward(self, pred, target):
        loss_dice = self.dice(pred, target, softmax=False)
        loss_ce = self.bce(pred, target[:].long())

        loss = self.wd * loss_dice + self.wb * loss_ce
        return loss
...
class GT_BceDiceLoss(nn.Module):
    def __init__(self, wb=1, wd=1):
        ...
        self.bcedice = BceDiceLoss(wb=1, wd=1, num_classes=2)
